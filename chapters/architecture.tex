...
A node is a physical computer that runs the \texttt{GNU/Linux}\footnote{\url{https://www.gnu.org}}\footnote{\url{https://kernel.org}}
...
Kubernetes-compliant distribution. Kubernetes, also known as K8s, is an open-source
solution for automating containerized application deployment, scaling, and
administration\cite{k8s}. The second service, Metrics Server, is a server that
constantly monitors the node, exposing hardware and operating system metrics. \\ %
...
A controller node is an essential component of the cluster, acting as a
...
A server handles all cluster nodes, user authentication and authorization, and
...
(turning on or off a node or displaying sensitive user information) must be protected
...
error or faulty condition, most typically an irreversible hardware failure of
...
and distributes container-based application images\cite{docker_registry}. \\ %
...
Cluster Autoscaler is uploaded and made available to the whole cluster during
the initialization phase. Because the registry is defined as a critical K8s service,
...
Figure \ref{fig:load_balancer} depicts an example schema of an Internal Load Balancer
in a cluster architecture. It is worth noting that the Internal Load Balancer
component is presented externally to the nodes to simplify the overall overview
but in reality, it is deployed directly on each active node.

\begin{figure}[htbp]
  \centering
  \includegraphics
  [width=.7\textwidth]{images/architecture/load_balancer_example.pdf}
  \caption{Internal Load Balancer schema example}
  \label{fig:load_balancer}
\end{figure}

There are three active nodes and one Internal Load Balancer in the cluster. The nodes'
...
The Internal Network uses one or more layer 2 network switches to connect each
...
requirement for routers. Without them, a considerable amount of resource waste
...
and managed in a cluster\cite{k8s}, is assigned its cluster-wide IP address.
...
the Container Network Interface (CNI)\cite{k8s}. CNI is composed of a specification
and libraries for designing plugins that configure network interfaces in Linux containers.
CNI is only concerned with the network connectivity of containers and removing
allocated resources when the container is destroyed\cite{cni}.
...
It employs several hardware components decommissioned by the University of Trento\footnote{\url{https://www.unitn.it}}
