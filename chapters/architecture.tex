...
provider. However, the overall architecture in question is much simpler and runs
on bare-metal servers. A bare-metal server is a physical computer that is used by
one consumer, or tenant, only\cite{https://wikipedia.org/wiki/Bare-metal_server}.
Therefore, an external load balancer can be replaced with an internal one that
is directly deployed in the K8s environment. An internal load balancer reduces the
overall resource waste and improves its maintainability. Moreover, as for the registry
and cluster autoscaler components, the internal load balancer is maintained
automatically operational and in a healthy state at all times by K8s.

Figure \ref{fig:load_balancer} show an example schema of how an internal load
balancer works in the cluster architecture. Note that the internal load balancer
component is shown external to the nodes to simplify the overview and to better understand
how it works but in reality, it is directly deployed on every node.

\begin{figure}[htbp]
  \centering
  \includegraphics
  [width=.7\textwidth]{images/recluster/load_balancer_example.png}
  \caption{Internal Load Balancer example}
