...
The Internal Network interconnects every physical node in the cluster by the use
of one or more Layer 2 network switches. To put it simply, the Internal Network
is mostly a device to device communication that does not involves other networks.
A network switch connects devices within the same network and forwards data packets
to and from those devices. A device can be another switch, a router, or a
computer. Unlike a router, a switch only sends data to the single device it is
intended for, not to networks of multiple devices\cite{https://www.cloudflare.com/learning/network-layer/what-is-a-network-switch}.
Since the Internal Network it is mostly a group of two or more connected devices,
and is a network contained within a small geographic area it is considered a LAN
(Local Area Network)\cite{https://www.cloudflare.com/learning/network-layer/what-is-a-lan}.

Thanks to the overall network design and the utilization of only L2 network switches,
the Internal Network is analogous to Layer 2, the Data Link Layer, of the ISO/OSI
communication model. The Data Link Layer is very similar to the Network Layer (L3),
except that the Data Link Layer facilitates data transfer between two devices on
the same network. The Data Link Layer takes packets from the Network Layer (L3) and
breaks them into smaller pieces called frames. Like the Network Layer, the Data Link
Layer is also responsible for flow control and error control in intra-network communication
wether the Transport Layer does flow control and error control for inter-network
communications\cite{osi}.

Within a LAN, due to the network configuration at Layer 2, the communication
between the internal devices is not based on IP addresses, used at Layer 3,
rather on MAC addresses. Therefore, network switches forward data based on the
destination MAC address. A MAC address (Media Access Control address) is a unique
sequence of numbers assigned to a network interface controller (NIC) that usually
encodes the manufacturer's registered identification number. A device may have
more than a single NIC, and each of them has its own unique MAC address. Even
though a MAC address is permanently stored in the NIC's hardware there are
software that allows to override it during the communications\cite{https://macaddress.io/faq/what-is-a-mac-address-after-all}.
One of the difference between IP addresses and MAC addresses is that the first
are dynamically assigned to each device and can change over time while the
latter are permament assigned. This difference is key; the device's MAC address
can be used to uniquelly identify a device in a network and automatically
bootstrap it. The latter technique is called Wake-on-LAN (WoL) and is extensively
used in the cluster by the server component. More information available in
section \ref{cha:implementation}.

If the Internal Network needs to communicate with other networks to access
different resources or perform external actions, a router device is needed.
Therefore, the router (Layer 3) is directly connected to a switch (Layer 2) that
in turn connects every node in the cluster. The latter connection between the
two layers, is seen as the border from the secure Internal Network to the unsecure
External Network. Moreover, the total number of switch and/or router devices in the
architecture is not fixed and can be increased to sustain better performance and
possible fault tolerance of the overall cluster network. If an external
communication is not needed, the overall architecture is then simplified and composed
by only L2 network switches removing the needs of routers. Without them, a significant
amount of resource waste is decreased, not only involving the pure hardware that
can be reutilized as worker nodes, but also in terms of the overall cluster power
consumption since a network switch, compared to a router, is more power-efficient\cite{}.

Finally, to easily identify nodes in the cluster with a friendly name a local domain
can be used (i.e., recluster.local). .local is a special-use domain name reserved
by the Internet Engineering Task Force (IETF)\footnote{\url{https://wikipedia.org/wiki/Internet_Engineering_Task_Force}}
so that it may not be installed as a top-level domain in the Domain Name System (DNS)\footnote{\url{https://wikipedia.org/wiki/Domain_Name_System}}
of the Internet. As such it is similar to the other special domain names. However,
.local is designated for use in link-local networking, so that DNS service may
be established without local installations of conventional DNS infrastructure on
LANs\cite{https://wikipedia.org/wiki/.local}. As such, a worker node in the cluster
can be easily identified with the name `worker.recluster.local` instead of its IP
