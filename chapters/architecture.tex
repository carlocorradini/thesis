...
without the need for local deployments of standard DNS infrastructure on LANs\cite{local_domain_name}.
As a result, instead of an IP address or MAC address, a worker node may be recognized
...
starting from the Data Link Layer (L2) up to the Application Layer (L7), which is
the highest. \\ %
...
The following information is essential for understanding the principles of K8s networking.
They were collected from the official Kubernetes website, which can be found at \url{https://kubernetes.io}.
