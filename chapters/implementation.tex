...
\begin{enumerate}
  \item A Cluster Autoscaler or Administrator entity submits a request to the GraphQL
    ...
    argument) and a \texttt{count} value higher than the current count. To allow
    the entity to perform the action, a valid \texttt{JWT} token must be supplied
  ...
  \item \texttt{N} nodes are selected starting at the top of the list, where the
    ...
    associated node's NIC. After that, the state of each node is updated to \texttt{BOOTING}.
    \newline
...
\begin{figure}[htbp]
  ...
  \caption{Upscaling scheme phases}
  \label{fig:upscaling}
\end{figure}
