...
MetalLB is a load-balancer implementation that uses standard routing protocols
for bare-metal Kubernetes clusters. \cite{https://metallb.universe.tf}. MetalLB
has a direct mapping to the Load Balancer component of the cluster architecture
and, since it is executed directly on and managed by Kubernetes itself, it is
also considered an internal Load Balancer that does not require any external
system to function.

--- LoadBalancer Service type --- Kubernetes by default does not offer a fully
fledged implementation of a network load balancer (Service of type LoadBalancer\footnote{\url{https://kubernetes.io/docs/concepts/services-networking/service/\#loadbalancer}})
...
The native Kubernetes implementation of network load balancers are only a set of
predefined interfaces that are specifically designed/mapped to be compatible with
the external and complex load balancers running inside huge and energy hungry data
centers of the various cloud providers. These interfaces does not represents
real implementations of network load balancers but are considered as proxies
that translates the various API/function calls from the Kubernetes ecosystem to
the specific cloud provider's load balancer and viceversa. Therefore, every
Kubernetes Services of type "LoadBalancer" in bare-metal clusters remain in a "pending"
state indefinitely, leaving every deployments that are leveraging the latter services
unreachable from outside the Kubernetes network.

Without the possibility of exploiting Kubernetes services of type LoadBalancer,
the only way that bare-metal clusters are left with for enabling external
traffic to establish connections with internal deployments are mainly two; they are
briefly described below. 1. NodePort: The Kubernetes control plane allocates a port
from a predefined range, by default from port 30000 to port 3276, and every node
in the cluster proxies the same port into a Service instance. Every node in the
cluster is configured to listen on the same assigned port and to forward traffic
to one of the ready endpoints associated with the Service. Any external traffic that
establishes a connection to any node of the cluster using the appropriate protocol
(i.e., TCP or UDP) and port (i.e. 8080 or 9999) is forwared to a corresponding
internal service instance. \cite{https://kubernetes.io/docs/concepts/services-networking/service/\#type-nodeport}
1. ExternalIPs: External IP addresses are routed to one or more cluster nodes. Network
traffic that ingresses the cluster with the external IP (as destination IP), on
the corresponding service port, is then routed to one of the Service endpoints.
ExternalIPs are not managed by Kubernetes itself and therefore is the
responsibility of the cluster administrator. \cite{https://kubernetes.io/docs/concepts/services-networking/service/\#external-ips}

Both of these service types have significant downsides which makes bare-metal
clusters not ideal for a production environment.

MetalLB offers a network load balancer implementation (service of type LoadBalancer)
that directly resides inside the Kubernetes environment/network and easily
integrates with standard network devices and protocols allowing Kubernetes
deployments/services to be accessible from external network traffic.

--- Address Allocation --- MetalLB is in charge of allocating and assigning IP
addresses based on predefined IP address pools manually configured by the
organization in charge of administrating the cluster. The IP addresses in the range
of the pools must not be already allocated and/or reachable within the cluster network
otherwise an IP address conflict occurs\footnote{\url{https://www.linksys.com/support-article?articleNum=132159}}.
After MetalLB is deployed and configured it automatically assign and unassign
individual addresses within the pools' range to Kubernetes services of type LoadBalancer.
Moreover, a service deployment can be arbitrarily configured to be assigned with
a specific IP address that is available within the range of a MetalLB pool and is
also not already assigned to another service such that it is always externally available
with the same IP address\cite{https://metallb.universe.tf/concepts}
