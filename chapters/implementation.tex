...
K3s is a lightweight Kubernetes distribution that is designed for production workloads
in environments where resource-constrained, high availability, unattended and remote
operations are crucial. K3s is the core of the cluster implementation where all
Kubernetes-related operations are managed and executed. K3s does not have a perfect
mapping to an architecture component since it is one of the key application that
compose both a Worker and Controller Node component.

K3s is packaged as a single small binary (less than 60MiB) that reduces the dependencies
and steps needed to install, run and auto-update a production Kubernetes cluster.
It can be executed on any Linux distributions that have a Kernel that meets the minimum
Kubernetes requirements, and is fully compatible with both the OpenRC and
systemd Init Systems.

K3s is a fully compliant Kubernetes distribution where most of the legacy, alpha
and cloud-provider-specific code has been removed to reduce the overall application
size and its minimal hardware requirements. Moreover, following the latter, K3s
has the following enhancements over the standard Kubernetes distribution: 1. All
required dependencies are pre-packaged within the single binary file requiring only
a minimal set of external dependencies (a modern Kernel and cgroup mounts). 1.
Lightweight storage backend based on sqlite as the default storage mechanism. Additional
solutions based on etcd, MySQL or PostgreSQL are also available. Both embedded and
external storage are supported. As written in section B, the cluster architecture
design is structured around a high availability model rather than a fault tolerance
approach. Therefore, is suggested to rely on etcd (embedded or external) storage
or directly use the cluster's database as external storage avoiding possible unnecessary
data replications and relyign on a single storage area that can be managed autonomously.
1. Secure by default with reasonable default configurations for lightweight
environments. K3s automatically manages the complexity of TLS(Transport Layer Security)
like certificates distributions. If security is not a concern or the cluster is located
in an Air-Gap environment, the security configuration can be relaxed to improve
the overall performance while also reducing resource usage.

K3s architecture have a clear distinction between the two possible node types (Controller
and Worker): 1. Server node A Server node is defined as a host running the K3s server
command (`k3s server`), with control-plane and datastore components managed by
K3s. The cluster's architecture Controller Node component, shown in section A, is
a logical mapping to the Server Node. 2. Agent node An Agent node is defined as
a host running the K3s agent command (`k3s agent`), without any datastore or control-plane
components. The cluster's architecture Agent Node component, shown in section B,
is a logical mapping to the Agent Node. Moreover, the architecture of K3s is
fully modular and can support different setup based on the cluster's usage and
final objective: 1. Single-server Setup with an Embedded Database A single-node K3s
server with an embedded SQLite database. Ech agent node is registered to the
same server node. A K3s user can manipulate Kubernetes resources by calling the K3s
API on the server node. This setup is suggested only for testing purposes and
not for real production deployments nor for the cluster implementation. 2. High-Availability
K3s Server with an Embedded Database An High-Availability (HA) K3s cluster is composed
of two or more Server nodes that serve the Kubernetes API and run other control plane
services. The Database is embedded in the measn that it is executed in the same
K3s instance on the same Server node. 3. High-Availability K3s Server with an External
Database Similar to High-Availability K3s Server with an Embedded Database setup
but the datastore is external to the Server Node. The K3s Server instance does
not run any database application but relies on an external running, and reachable,
database.

K3s distribution has been chosen among a multitude of different Kubernetes
distributions, including the official one, for four main reasons: 1. Minimum hardware
requirements of K3s are extremely minimal and very easy to achieve even for old systems:
1.1 Server node requires 1 GiB of memory and 1 CPU core. As written in section A,
server (controller) node's minimal hardware requirements must be adapted
following the size of the cluster. It is impossible to imagine a cluster with hundreds
of worker nodes that is managed by a single server node with only 1GiB of memory
and 1 CPU core. 1.2 Agent node hardware requirements are even lower: 512 MiB of memory
and 1 CPU core. A Raspberry Pi 3 Model B single-board computer, released in February
2016(7 years ago as of time of writing), is fully compatible with K3s and can be
used as a worker node without any particular problem.

As a comparison, the official Kubernetes distributions, named kubeadm, requires
at least 2 GiB of memory and 2 CPU core.

2. K3s is extremely modular, configurable and easy to use. As an example, to bootstrap
a minimal Kubernetes cluster in a Single-server with an Embedded Database setup with
K3s is straightforward: 2.1. Start the Server Node with a custom token (used to
join a server or agent nodes to the cluster): k3s server --token <TOKEN> 2.1. Start
the Agent Node with the custom token and the server address: k3s agent --token <TOKEN>
--server <SERVER\_ADDRESS>

3. K3s can be installed in an Air-Gap environment with minimal effort. The only
additional steps required with respect to a standard installation to have a
fully functioning cluster are the requirement to deploy a private registry (see section
A) and the necessity to manually deploy specific container images to each node. By
default, the latter two requirements are already available in the cluster
implementation's bundle file. Therefore, an organization that needs to deploy a
cluster in an Air-Gap environment have been already provided with all the
necessary files and applications without requiring any addiotional data transfer
involving an internet connecion.

4. K3s project has been accepted to the Cloud Native Computing Foundation (CNCF)
on August 19th 2020 with Sandbox maturity level. CNCF is the vendor-neutral hub of
cloud native computing hosting critical components of the global technology infrastructure
dedicated to making cloud native ubiquitous. One of the key aspects of CNCF is
that it certifies Kubernetes software conformance ensuring that every vendor's
version of Kubernetes supports the required APIs, as do open source community versions.
The latter implies full compatibility with all available Kubernetes products and
software enabling the interchangiability of different components within the
cluster implementation reflecting different organizations core aspects.
