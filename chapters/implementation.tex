...
Docker Registry is a stateless, highly scalable server side application that stores
and distributes container-based application images. A registry is a storage and
content delivery system, holding named Docker images, available in different
tagged versions. Docker registry has an direct mapping to the Registry component
of the cluster architecture.

Storage The registry must save the available images onto persistent storage. The
storage is delegated to drivers where the default driver is the local POSIX
filesystem of the node executing the registry instance. This is not suitable and
can lead to losing all the available images. Therefore, the cluster can be equipped
with additional storage drivers to fullfill possible different needs and requirements.
Note however that the chosen driver(s) must be compatible with the available cluster
environment, nodes' hardware and/or fullfills certain additional requirements.
An example, from the same developers of K3s, of a possible solution is Longhorn\footnote{\url{https://longhorn.io}}:
an Open Source, lightweight, reliable and easy-to-use distributed block storage system
for Kubernetes. Scrivi SSD consigliate!

FOTO!

Image Naming An image name is composed of slash-separated name fields, optionally
prefixed with a registry hostname, that comply with standard DNS (Domain Name System)
rules, and optionally postfixed with a tag name that identify different versions
of the same series of images. If a hostname is present, it may optionally be followed
by a port number in the format ":<PORT>". If the registry hostname is not specified;
Docker, Kubernetes, podman and almost every application dealing with container
images, uses the Docker's public registry (Docker Hub\footnote{\url{https://hub.docker.com}})
located at "docker.io" by default. If a tag name is not specified, it is automatically
defaulted the value "latest" meaning that it is identified the latest available release
version of the image. As an example, the image name `node:18` (short form of `docker.io:443/node:18`)
identifies the container image of Node.js version 18 available on the Docker public
registry at port 443. Instead, the name `registry.recluster.local:5000/node:18`
equally identifies the container image of Node.js version 18 but available on a
local private registry at port 5000. Therefore, whenever in the cluster there is
the need of referencing container images on a registry that is not the default one
it must be specified the full hostname.

Mapping hostname to IP address A local registry deployment implies that in the cluster
there must be a known mapping between the registry hostname to the IP address where
the registry instance is running. There are three solutions available that are
shown in the list below: 1. Relying on a custom configuration file/entry of the chosen
Kubernetes distribution that instructs the container runtime about the mapping.
K3s does support this feature by default via a configuration file named "registries.yaml"
that must be saved under the directory "/etc/rancher/k3s" on each node that compose
the Kubernetes cluster. 1. Adding an entry in the local "/etc/hosts" file on every
node of the cluster. 1.Deploying a DNS component that must continuously running in
the cluster. This solution implies that all nodes in the cluster must be
configured to know about the local DNS by manually modifying the "/etc/resolv.conf"
or automatically using DHCP (Dynamic Host Configuration Protocol).

All three solutions have advantages and disadvantages. If the cluster size is
relatively small and the IP address of the local registry does not change over
time it is suggested to simply use the first or second solutions depending if
the chosen Kubernetes distribution support the feature. However, if the cluster
is composed with multiple nodes or the IP address of the registry can change
frequently or there is the necessity to have additional hostname to IP mapping, then,
having a DNS component is far way the best approach. Nevertheless, the final
decision on what is the best solution for the mapping relies on the organization
running the cluster: nothing forbids, even for a really small cluster, to deploy
a DNS component. In reCluster, since it relies on K3s, it is composed by a few
number of nodes and the IP address of the registry never changes thank to
MetalLB (see section A), the decision has relapsed to the first solution. More
