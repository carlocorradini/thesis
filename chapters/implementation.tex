...
\begin{lstlisting}[language=shell, morekeywords={[2]{depend, start_pre, want, after, sourcex}}, morekeywords={[4]{name, description, supervisor, command, command_args, output_log, error_log, pidfile, respawn_delay, respawn_max}}, xleftmargin=\parindent, caption=TODO]
  #!/sbin/openrc-run
  ...
  if [ -f "/etc/rancher/k3s/k3s.env" ]; then sourcex "/etc/rancher/k3s/k3s.env"; fi
  set +o allexport
\end{lstlisting}
...
\begin{lstlisting}[language=ini, morekeywords={Description, Documentation, Wants, After, WantedBy, Type, ExecStartPre, ExecStart, EnvironmentFile, KillMode, Delegate, TimeoutStartSec, Restart, RestartSec}, xleftmargin=\parindent, caption=TODO]
  [Unit]
  Description=Lightweight Kubernetes
  ...
  [Service]
  Type=notify
  ExecStartPre=/usr/bin/env sh -c 'rm -f "/tmp/k3s.*"'
